...
def get_usuario(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id_usuario, Nombre, Rol, Telefono FROM Usuarios WHERE id_usuario = %s", (id,))
    row = cursor.fetchone()
    conn.close()
    if not row:
...
def actualizar_suministro(id):
    ...
    cursor.execute(
        "UPDATE Suministros SET Nombre=%s, Tipo=%s, Cantidad=%s, Precio=%s, TotalCosto=%s WHERE id_sum=%s",
...
def get_producto(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id_prod, Codigo, Nombre, Descripcion, Stock_ch, Stock_m, Stock_g, Precio, id_usuario FROM Productos WHERE id_prod = %s", (id,))
    row = cursor.fetchone()
    conn.close()
    if not row:
...
def eliminar_producto(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Productos WHERE id_prod = %s", (id,))
    conn.commit()
    conn.close()
...
def crear_venta():
    ...
    cursor.execute(
        "INSERT INTO Ventas (id_pago, id_usuario, Total, Fecha, Devuelta, Origen) VALUES (%s,%s,%s,NOW(),%s,%s)",
    ...
    for item in data.get('items', []):
        ...
        if col:
            cursor.execute(f"UPDATE Productos SET {col} = {col} - %s WHERE id_prod = %s",
...
def get_apartado(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id_apart, Nom_cliente, Telefono, Anticipo, Total_ap, Restante, Fecha, Estado FROM Apartados WHERE id_apart = %s", (id,))
    row = cursor.fetchone()
    if not row:
        conn.close()
...
def crear_apartado():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO Apartados (Nom_cliente, Telefono, Anticipo, Total_ap, Fecha, Estado) VALUES (%s,%s,%s,%s,NOW(),%s)",
...
def eliminar_apartado(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Apartados WHERE id_apart = %s", (id,))
    conn.commit()
    conn.close()
...
def get_mayoreo(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id_clm, n_pedido, Nombre, Telefono, Total, Fecha FROM Mayoreos WHERE id_clm = %s", (id,))
    row = cursor.fetchone()
    if not row:
        conn.close()
...
def crear_mayoreo():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO Mayoreos (n_pedido, Nombre, Telefono, Total, Fecha, id_usuario) VALUES (%s,%s,%s,%s,NOW(),%s)",
...
def eliminar_mayoreo(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Mayoreos WHERE id_clm = %s", (id,))
    conn.commit()
    conn.close()
...
def crear_suministro():
    ...
    total = data['cantidad'] * data['precio']
    cursor.execute(
        "INSERT INTO Suministros (Nombre, Tipo, Cantidad, Precio, TotalCosto, Fecha, id_usuario) VALUES (%s,%s,%s,%s,%s,NOW(),%s)",
...
def eliminar_suministro(id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Suministros WHERE id_sum = %s", (id,))
    conn.commit()
    conn.close()
